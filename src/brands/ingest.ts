...
import { parseDesignMd as parseDesignSpecFile } from "../parser/design-parser.ts";
...
/**
 * Ingest every `*.md` in `dir` (recursive) by parsing each file with the
 * Phase A `parseDesignMd(filePath)` (front matter + prose scan) and bridging
 * the resulting `DesignSpec` to `BrandTokens` via `specToBrandTokens`. This is
 * the canonical ingest path shared with the CLI `export`/`inspect` commands,
 * so the preview showroom and the exported projects render the same themes.
 */
export async function ingestDirSpecs(dir: string): Promise<IngestResult> {
  ...
  for (const f of files) {
    try {
      const spec = await parseDesignSpecFile(f);
      let bid = spec.id;
      let n = 1;
